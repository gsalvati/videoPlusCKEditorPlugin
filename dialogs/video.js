CKEDITOR.dialog.add( 'video', function ( editor )
{
	...
	function commitSrc( videoNode, extraStyles, videos, tracks )
	{
	...
	function commitTrack( videoNode, extraStyles, videos, tracks )
	{
		var match = this.id.match(/(\w+)(\d)/),
			id = match[1],
			number = parseInt(match[2], 10);

		var track = tracks[number] || (tracks[number]={});
		track[id] = this.getValue();
	}
	...
	function loadSrc( videoNode, videos, tracks )
	{
		var match = this.id.match(/(\w+)(\d)/),
			id = match[1],
			number = parseInt(match[2], 10);
	...
	function loadTrack( videoNode, videos, tracks )
	{
		var id = this.id.replace('dsrc','src');
	...
	return {
		...
		onShow : function()
		{
			...
			if ( fakeImage && fakeImage.data( 'cke-real-element-type' ) && fakeImage.data( 'cke-real-element-type' ) == 'video' )
			{
				...
				var videoNode = editor.restoreRealElement( fakeImage ),
					...
					trackList = videoNode.getElementsByTag( 'track', '' );
				if (sourceList.count()==0)
					sourceList = videoNode.getElementsByTag( 'source', 'cke' );

				if (trackList.count()==0)
					trackList = videoNode.getElementsByTag( 'track', 'cke' );
				...
				for ( var i = 0, length = trackList.count() ; i < length ; i++ )
				{
					var item = trackList.getItem( i );
					tracks.push( {src : item.getAttribute( 'src' ), srclang: item.getAttribute( 'srclang' ), kind:"subtitles", label: item.getAttribute( 'label' ) } );
				}

				this.videoNode = videoNode;

				this.setupContent( videoNode, videos, tracks );
		...
		onOk : function()
		{
			...
			var extraStyles = {}, videos = [], tracks = [];
			this.commitContent( videoNode, extraStyles, videos, tracks );
			...
			for(var i=0; i<tracks.length; i++)
			{
				...
				if ( !track || !track.dsrc )
					continue;
				innerHtml += '<cke:track src="' + track.dsrc + '" srclang="' + track.srclang + '" label="' + track.label + '" kind="subtitles" />';
				links += link.replace('%src%', track.dsrc).replace('%srclang%', track.srclang).replace('%label%', track.label);
			}
			
			console.log(innerHtml + fallbackTemplate.replace( '%links%', links ));
